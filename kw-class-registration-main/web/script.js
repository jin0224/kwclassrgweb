// ===== State Variables (matching original C# exactly) =====
let sec = 50;
let getNum = 0;
let numOfSub = 6;
let isStarted = false;
...
let isBlocked = false; // prevents interaction during delays

// Course data generated on load
let courses = [];
...
// ===== Custom MessageBox (mimics WinForms MessageBox) =====
...
// ===== Timer Tick (matching original exactly) =====
...
// ===== Practice Start (matching original exactly) =====
function practiceStart() {
    ...
    for (let i = 0; i < numOfSub; i++) {
        isFull.push(randInt(0, 10) < 2);  // 20% chance full
        isDone.push(false);
...
// ===== Query Button Click (matching original exactly) =====
async function onQueryClick(index) {
    ...
    // Show loading overlay (simulates Thread.Sleep UI blocking)
    isBlocked = true;
...
// ===== Enrol Button Click (matching original exactly) =====
